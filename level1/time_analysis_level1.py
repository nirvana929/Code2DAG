...
def main() -> int:
    ...
    ap.add_argument("--base-dir", type=Path, default=Path(__file__).resolve().parents[1], help="Project base dir (contains intermediate_results/)")
    ap.add_argument("--base-name", required=True, help="Base name under intermediate_results/, e.g. cpu4_thread10_fifo")
    ...
    seg_json = base_dir / "intermediate_results" / base_name / "level1" / "stage1" / "segments_stage1.json"
    if not seg_json.exists():
        raise SystemExit(f"missing segments json: {seg_json}")

    dag_json = base_dir / "intermediate_results" / base_name / "level1" / "stage1" / "dag_stage1_seg.json"
    ...
    out_root = base_dir / "intermediate_results" / base_name / "level1" / "timing" / project_name
    if out_root.exists():
