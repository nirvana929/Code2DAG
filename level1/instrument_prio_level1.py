...
def main ()->int :
    ...
    ap .add_argument ("--project-name",default =None ,help ="Output project name under time_analysis_level1_prio/")
    ...
    schedule_path =args .schedule or (base_dir /"intermediate_results"/base_name /"level1"/"schedule"/"lpf_segment"/"schedule_seg.json")
    priorities =_load_schedule (schedule_path )

    segments_json =base_dir /"intermediate_results"/base_name /"level1"/"stage1"/"segments_stage1.json"

    ts =time .strftime ("%Y%m%d_%H%M%S")
    out_root =base_dir /"intermediate_results"/base_name /"level1"/"experiments"/f"lpf_segment_{ts}"/project_name 
    if out_root .exists ():
    ...
    # Prio: rename the main Source File to distinguish it, and only add priority switching to this copy (total processing time of wrap_main.c)
