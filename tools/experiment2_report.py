#!/usr/bin/env python3
"""\nGenerate consolidated tables and figures for experiment2 results (VM & RK3588).\nOutputs are written to: mycallyplus_v1/experiment/experiment2experiment summary/\n\nData sources (read-only):\n- mycallyplus_v1/experiment/experiment2 virtual machine experimentresult summary/*/run_results_*.txt (and summary.json if present)\n- mycallyplus_v1/experiment/experiment2（rk3588）experimentresult/*/run_results_*.txt (and summary.json if present)\n"""
import json 
import math 
import os 
import re 
...
SRC_DIRS ={
"vm":BASE /"experiment"/"experiment2 virtual machine experimentresult summary",
"rk3588":BASE /"experiment"/"experiment2（rk3588）experimentresult",
}
OUT_DIR =BASE /"experiment"/"experiment2experiment summary table"
...
def save_pretty_tables (df_exp ,df_long ,df_pair ,df_plat ):
    ...
    lines =[]
    lines .append ("# experiment2 summary table (beautified version overview)\n")
    lines .append ("## experiment_index (first 10 rows)\n")
    lines .append (df_exp_r .head (10 ).to_markdown (index =False ))
    lines .append ("\n## platform_summary\n")
    lines .append (df_plat_r .to_markdown (index =False ))
    lines .append ("\nData File:\n")
    lines .append (f"- Excel: `{xlsx_path.relative_to(BASE)}`\n- Parquet: *.parquet in the same directory (generated if the environment supports it)\n- original CSV remains in `tables/`")
