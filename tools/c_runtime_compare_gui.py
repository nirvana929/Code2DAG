    ...
    def _run_one (self ,task :Task )->None :
        ...
        try :
            ...
            # Temporary shortage: keep it queued and try again later.
                task .status ="queued"
                task .phase ="wait_cpu"
                task .message =f"wait CPU: Need {task.cores_per_task} core, current idle {self._cpu_pool.free_count()}"
            ...
            if task .use_sudo and not _has_passwordless_sudo ():
                task .status ="error"
                task .message ="enabled sudo runs but current is not available (please use sudo to start this tool, or configure sudo NOPASSWD)."
                self ._on_update (task )
                return 
            ...
            task .status ="done"
            task .phase ="done"
            task .message =f"Completed：baseline mean={baseline_stat['mean_s']:.3f}s, prio mean={prio_stat['mean_s']:.3f}s"
...
class App (tk .Tk ):
    def __init__ (self )->None :
        super ().__init__ ()
        self .title ("C program comparison timing tool (baseline vs prio)")
    ...
    def _build_ui (self )->None :
        ...
        # Top controls
        top =ttk .LabelFrame (frm ,text ="Create a new experiment task")
        top .pack (fill =tk .X )
        ...
        def pick (var :tk .StringVar )->None :
            p =filedialog .askopenfilename (title ="Select C File",filetypes =[("C file","*.c"),("All","*.*")])
            if p :
                var .set (p )
        ...
        ttk .Button (row0 ,text ="Select",command =lambda :pick (self .var_baseline )).pack (side =tk .LEFT )
        ...
        ttk .Button (row1 ,text ="Select",command =lambda :pick (self .var_prio )).pack (side =tk .LEFT )
        ...
        ttk .Label (row2 ,text ="Number of repetitions:").pack (side =tk .LEFT ,padx =(12 ,0 ))
        ttk .Entry (row2 ,textvariable =self .var_repeats ,width =6 ).pack (side =tk .LEFT ,padx =6 )
        ttk .Label (row2 ,text ="Number of cores per task:").pack (side =tk .LEFT ,padx =(12 ,0 ))
        ttk .Entry (row2 ,textvariable =self .var_cores ,width =6 ).pack (side =tk .LEFT ,padx =6 )
        ttk .Checkbutton (row2 ,text ="Run with sudo (requires root or sudo -n)",variable =self .var_use_sudo ).pack (side =tk .LEFT ,padx =(12 ,0 ))
        ttk .Checkbutton (row2 ,text ="Single concurrent Queue mode",variable =self .var_queue_mode ).pack (side =tk .LEFT ,padx =(12 ,0 ))

        row3 =ttk .Frame (top )
        row3 .pack (fill =tk .X ,padx =8 ,pady =6 )

        ttk .Label (row3 ,text =f"Online CPUs: {len(self.cpu_list)} (online={_format_cpu_set(self.cpu_list)})").pack (side =tk .LEFT )
        self .lbl_cpu_free =ttk .Label (row3 ,text ="")
        self .lbl_cpu_free .pack (side =tk .LEFT ,padx =12 )

        ttk .Button (row3 ,text ="open experiment directory",command =lambda :_open_in_file_manager (self .base_dir /"experiment")).pack (side =tk .RIGHT )

        ttk .Button (row3 ,text ="queued",command =self ._enqueue_task ).pack (side =tk .RIGHT ,padx =8 )

        # Task list
        mid =ttk .LabelFrame (frm ,text ="Background tasks")
        ...
        ttk .Button (btns ,text ="open selected task resultsdirectory",command =self ._open_selected_out_dir ).pack (side =tk .LEFT )
        ttk .Button (btns ,text ="copy selected task resultpath",command =self ._copy_selected_out_dir ).pack (side =tk .LEFT ,padx =8 )
    ...
    def _refresh_cpu_label (self )->None :
        self .lbl_cpu_free .configure (text =f"Number of idle cores: {self.cpu_pool.free_count()} / {self.cpu_pool.total_count()}")
    ...
    def _enqueue_task (self )->None :
        try :
            ...
            if not baseline .exists ()or baseline .suffix .lower ()!=".c":
                raise ValueError ("baseline C File is invalid")
            if not prio .exists ()or prio .suffix .lower ()!=".c":
                raise ValueError ("prio C File is invalid")
            ...
            if repeats <1 :
                raise ValueError ("The number of repetitions must >= 1")
            if cores <1 :
                raise ValueError ("The number of cores per task must >= 1")
        except Exception as e :
            messagebox .showerror ("ParameterError",str (e ))
            return 

            # Warn if CPU cores not enough to even run one task.
        if cores >len (self .cpu_list ):
            messagebox .showwarning ("Insufficient CPU",f"Need {cores} core, but Online CPUs only have {len(self.cpu_list)}.")
            return 
        ...
        self .status_bar .configure (text =f"Queued: {task_id}")
        self ._refresh_cpu_label ()
    ...
    def _copy_selected_out_dir (self )->None :
        ...
        self .status_bar .configure (text =f"Copypath: {t.out_dir}")
