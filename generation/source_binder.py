...
def _extract_function_region(src: str, func: str) -> str:
    # Prefer real function definition, avoid forward declarations.
    # Example to match:
    ...
    if m is not None:
        idx = m.start()
        lbrace = src.find("{", m.start())
    else:
        # Fallback: scan each "<func>(" occurrence and choose the first one
        # whose parameter list is followed by "{" before ";".
        idx = -1
        lbrace = -1
        start = 0
        needle = func + "("
...
def bind_from_source(expand_path: Path, owner_function: str, source_override: Optional[Path] = None) -> List[str]:
    ...
    if not lines:
        return []
    src_path = source_override if source_override else _guess_source_from_expand(lines, expand_path)
...
def create_targets_from_source(expand_path: Path, owner_function: str, source_override: Optional[Path] = None) -> List[str]:
    """Return task function list in pthread_create appearance order."""
    lines = _read_expand(expand_path)
    if not lines:
        return []
    src_path = source_override if source_override else _guess_source_from_expand(lines, expand_path)
    ...
    try:
        ...
        return []
    region = _extract_function_region(src_text, owner_function)
    creates, _joins = _scan_calls(region)
    return [task for _handle, task in creates if task]
