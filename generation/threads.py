...
"""Thread edge patching for the modernized interface.

Only used by tests and auxiliary tooling. GUI/CLI thread edges still come from
the legacy pipeline.
"""
...
def infer_thread_edges(graph: CallGraph) -> List[Tuple[str, str]]:
    """Infer ``tail -> pthread_join`` edges from the modernized cache."""
