...
def load_thread_metrics (json_path :Path )->Dict [str ,ThreadMetric ]:
    """Extract \"summary by thread\" count/total_ns from various JSON structures.\n\n    support：\n    1) thread_time_summary.json: {thread: {total_ns, count}}\n2) time_result.json: {callsite: {total_ns, count, ...}} -> aggregate by callsite prefix thread\n3) mycalls_meta(_internal).json: {thread: {node: {total_ns, count, ...}}} -> Aggregate by top-level thread\n    """
    data =json .loads (json_path .read_text (encoding ="utf-8"))
    if not isinstance (data ,dict ):
        raise ValueError ("The top level of JSON must be an object (dict)")
    ...
    raise ValueError ("Unrecognized JSON structure (Not found parseable threadstatistics field)")


def load_trace_events (json_path :Path )->List [TraceEvent ]:
    """readthreadcall trace（thread_trace.json）。\n\nExpected structure: [{key,start_ns,dur_ns}, ...]\n    """
    data =json .loads (json_path .read_text (encoding ="utf-8"))
    if not isinstance (data ,list ):
        raise ValueError ("trace JSON The top level must be an array (list)")
    out :List [TraceEvent ]=[]
    for i ,row in enumerate (data ):
        ...
        try :
            ...
            raise ValueError (f"trace JSON field type {i+1} is illegal")from None 
    if not out :
        raise ValueError ("trace JSON is empty or has no valid events")
    return out 
...
time_analysis_root :Optional [Path ]=None ,
    ...
    """Selectstatisticsgraphsavedirectory (preferred to intermediate_results/<base>/time_analysis/<project>/)."""
    json_path =json_path .expanduser ().resolve ()

    # If selected JSON is already inside .../intermediate_results/<base>/time_analysis/<project>/..., use that project dir.
    parts =list (json_path .parts )
    if "intermediate_results"in parts :
        idx =parts .index ("intermediate_results")
        if idx +2 <len (parts )and "time_analysis"in parts [idx +2 :]:
            j =parts .index ("time_analysis",idx +2 )
            if j +1 <len (parts ):
                return Path (*parts [:j +2 ])
            return Path (*parts [:j +1 ])

            # If GUI provided time_analysis_root (intermediate_results/<base>/time_analysis), choose newest project dir.
    if time_analysis_root :
        root =time_analysis_root .expanduser ().resolve ()
        if root .name =="time_analysis":
            candidates =[p for p in root .iterdir ()if p .is_dir ()]
            if candidates :
                return max (candidates ,key =lambda p :p .stat ().st_mtime )
        if root .parent .name =="time_analysis":
            return root 
...
filename :str ="thread_call_gantt.png",
    ...
    """Render threadcallGantt chart (divide threads by key prefix, share timeline across threads)."""
